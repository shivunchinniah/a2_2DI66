...
#     def __init__(self, env, name, ledger, total_bays, queue_limit, service_time_dist, bypass_func=None):
#         super().__init__(env, name, ledger)
...
#         return len(self.fifo) < self.queue_limit
        
#     def _get_capacity_cost(self, entity):
#         # Bypassing vehicles drive down the aisle, consuming 0 bays
#         if self.bypass_func and self.bypass_func(entity):
#             return 0
            
#         cost = 2 if entity.vehicle_size == VehicleSize.BIG else 1
        
#         # Cap the cost to prevent deadlocks at single-lane choke points
...
#                     st = 10.0 # Drive through time in seconds
#                 else:
...
#             self._try_service()
#         else:
#             self.blocked_entities.append((entity, cost))
...
#             self.blocked_entities.remove(item)
